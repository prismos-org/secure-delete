...
/* if the blocksize on the filesystem is bigger than the on compiled with, enlarge! */
    // filestat.st_blksize Is a signed long int while bufsize Is an unsigned long int. Explicitly converting bufsize to long int.
    if (filestat.st_blksize > (long int)bufsize) {
...
void cleanup(int sig) {
    fprintf(stderr,"Terminated by signal %i. Clean exit.\n", sig);
    if (fd >= 0)
    	close(fd);
    FLUSH;
    exit(1);
